...
export default function AnimalProductForm({
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    await supabase.from("animal_product_logs").insert({
      ...
      animal_species: flockId
        ? (flocks.find(f => f.id === flockId)?.name ?? null)
        : null,
    ...
    // 2. Hvis slagtedyr + individuelle dyr valgt
    if (productType === "slagtedyr" && selectedAnimals.length > 0) {
      // Opret slaughter-event pr. dyr
      await supabase.from("animal_events").insert(
        selectedAnimals.map(animalId => ({
          ...
          event_date: date,
          data: {
            quantity_kg:    Number(qty) / selectedAnimals.length,
        ...
        <form onSubmit={handleSave} className="rounded-xl p-4 space-y-3"
          ...
          {isSlagtedyr && flockAnimals.length > 0 && (
            <div>
              ...
              <div className="mt-1 rounded-lg overflow-hidden divide-y divide-white/5"
                ...
                {flockAnimals.map(a => {
                  ...
                  return (
                    <button
                      ...
                      style={{ background: isSelected ? "rgba(163,230,53,0.08)" : "transparent" }}
                    >
                      <div className="w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors"
                        style={{ borderColor: isSelected ? "#a3e635" : "rgba(255,255,255,0.2)", background: isSelected ? "rgba(163,230,53,0.2)" : "transparent" }}>
                        {isSelected && <Check size={10} style={{ color: "#a3e635" }} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-sm text-earth-100">{a.ear_tag}</span>
                        {a.name && <span className="text-earth-400 text-xs ml-1.5">{a.name}</span>}
                      </div>
                      <span className="text-[10px] text-earth-500 capitalize flex-shrink-0">{a.species}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <button type="submit" disabled={saving || !qty}
            ...
            {saving ? "Gemmer…" : isSlagtedyr && selectedAnimals.length > 0
              ? `Gem + registrér ${selectedAnimals.length} dyr som slagtet`
              : "Gem registrering"}
          </button>
        </form>
      )}
