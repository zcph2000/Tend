  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link href="/farming/polytunnel/new" className="btn-primary flex items-center gap-2 px-3 py-2 text-sm">
          <Plus size={16} /> Ny
        </Link>
      </div>
      ...
      {(tunnels ?? []).length === 0 ? (
        <div className="card flex flex-col items-center py-10 gap-3 text-center">
          ...
          <Link href="/farming/polytunnel/new" className="btn-primary text-sm px-4 py-2">Opret polytunnel</Link>
        </div>
      ) : (
        <div className="space-y-3">
          {(tunnels ?? []).map((t) => {
            ...
            return (
              <Link key={tunnel.id} href={`/farming/polytunnel/${tunnel.id}`} className="card block hover:brightness-110 transition-all">
